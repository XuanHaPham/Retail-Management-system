...
export default {
  ...
  methods: {
    refresh: function() {
      this.loading = true;
      this.current = null;
      openapi(methods.GET, routes.BILLSOUTPUT).then(data => {
        this.loading = false;
        this.items = data;
      })
    },
    ...
    //   openapi(methods.PUT, routes.BILLSOUTPUT, this.current).then(data => {
    //     if (data.Message) {
    ...
    add: function() {
      ...
      openapi(methods.POST, routes.BILLSOUTPUT, this.current).then(data => {
