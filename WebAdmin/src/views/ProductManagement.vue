  ...
  <div>
    ...
    <b-row v-if="!loading" class="animated fadeIn">
      <b-col :lg="12">
        ...
        <b-button class="mb-2" @click="create" variant="primary">Add new product</b-button>
      </b-col>

      <b-col :lg="current ? viewMembers ? 4 : 6 : 12">
        <c-table :table-data="items" :fields="fields" hover caption="Products" :rowClicked="rowClicked"></c-table>
      </b-col>
      ...
      <b-col :lg="viewMembers ? 4 : 6" v-if="current" class="animated fadeIn">
        <b-card class="card-accent-info" :header="'Detail of `' + current.Name + '`'">
          <b-form>
            ...
            <b-form-group v-for="field in fields" :key="field.key" v-if="['id', 'status', 'creator', 'timeCreated'].indexOf(field.key) === -1">
              <b-input-group>
...
export default {
  ...
  methods: {
    ...
    update: function() {
      ...
      openapi(methods.PUT, routes.PRODUCTS, this.current).then(data => {
        if (data.Message) {
    ...
    remove: function() {
      ...
      openapi(methods.DELETE, routes.DELPRODUCTS, this.current).then(data => {
